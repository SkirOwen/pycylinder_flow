...
C = scipy.io.loadmat('Re100Data_Take2/u_vs_t_downstream_idx300_idx50_v1.mat');
F = scipy.io.loadmat('Re100Data_FineGrid_Take2/u_vs_t_downstream_idx600_idx100_v1.mat');

# C.ti = [1:length(C.up)];
# F.ti = [1:length(F.up)];

C_ti = np.arange(1, len(C['up']) + 1)
F_ti = np.arange(1, len(F['up']) + 1)
##
