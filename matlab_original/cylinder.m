...
for cycle = 0:maxT
    ...
    if (mod(cycle,tPlot)==0)
        ...
        u(bbRegion) = nan;
        imagesc(u'); axis equal off; drawnow
        %printf('%d of %d \n', cycle, maxT)
        vx = reshape(ux,lx,ly);
        vy = reshape(uy,lx,ly);
    ...
    vx = reshape(ux,lx,ly);
    vy = reshape(uy,lx,ly);
    u = sqrt(vx(2*50,2*25).^2 + vy(2*50,2*25).^2); %no idea what the values in vx(..,..) should be. Just picking as smaller than the overall window size
    up(cycle+1) = u;
end
save('TestSymVars.mat', 'bbRegion', 'x', 'y','obst_x','obst_y','obst_r','nu','Re','uMax','lx','ly')
